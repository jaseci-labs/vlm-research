...
if __name__ == "__main__":
    login(token="hf_XXXXXXXXXXXXX")  # Replace with your Hugging Face token

    wandb.init(
        project="Distillation with RSICD",
        name="Gemma-finetuned-and-base-responses"
    )

    # Load both models
    finetuned_model, finetuned_tokenizer = load_model("Malitha/Gemma3-4B-rsicd-phase-3-exp-1")
    base_model, base_tokenizer = load_model("unsloth/gemma-3-4b-it")

    question = "Analyze the remote sensing image."
    ...
    for response in data:
        ...
        teacher_response = json.dumps({
            "visual_scene": response["visual_scene"],
