...
wandb.init(
    ...
    name="Model 8 bit quantization-Dis",
    group="gemma tests",
...
HUGGINGFACE_HUB = "Malitha/Gemma3-4B-car-damage-model-8bit"
...
def upload_to_huggingface_hub(model, processor):
    model.push_to_hub(HUGGINGFACE_HUB)
    processor.push_to_hub(HUGGINGFACE_HUB)
...
train_data, val_data, test_data = dataset_split(os.path.join(IMAGES_PATH, JSON_FILE_PATH), test_size=0.2, random_state=42)
...
wandb.log({"peak_reserved_memory_phase_1_GB": used_memory_phase_1})
...
wandb.log({"peak_reserved_memory_phase_2_GB": used_memory_phase_2})
...
# Load both models
finetuned_model, finetuned_tokenizer = load_model(HUGGINGFACE_HUB)
