#!/usr/bin/env python3
"""
Fine-tune Gemma-3-12B-IT vision-language model for Flickr30k captioning task.
...
def finetune_model(
    ...
    wandb_project: str = "flickr30k-finetuning",
    ...
    """
    Fine-tune a vision-language model for Flickr30k captioning.

    Args:
        model_name: Base model identifier (e.g., unsloth/gemma-3-12b-it)
        train_dataset_path: Path to training dataset (Flickr30k split)
        eval_dataset_path: Path to evaluation dataset (Flickr30k split)
        save_dir: Directory to save fine-tuned model
        prompt: Instruction prompt for the model (e.g. CoT caption prompt)
    ...
    if use_wandb:
        try:
            ...
            print("   3. Disable WandB with --use-wandb=False")
    ...
    def convert_to_conversation(sample):
        """Convert Flickr30k sample to conversation format for Unsloth.

        The Flickr30k dataset (your processed version) has:
        - 'image': PIL Image or bytes that may need decoding
        - 'caption': list of 5 captions OR a single caption string
        """
        ...
        # If this is a list of 5 captions, pick one (e.g. random)
        if isinstance(captions, list):
            if len(captions) == 0:
                target_caption = ""
            else:
                # You can use captions[0] instead if you want deterministic behavior
                target_caption = captions[0]
        else:
            target_caption = str(captions)
    ...
    save_steps = eval_steps  # Make save_steps equal to eval_steps
    ...
    print(
        f"Peak reserved memory for training: "
        f"{used_memory_for_lora} GB ({lora_percentage}%)"
    )
    ...
    if upload_to_hf and hf_token and repo_id:
        ...
        try:
            ...
            print(
                f"✅ Model uploaded successfully to: "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Fine-tune VLM for Flickr30k captioning"
    )
    ...
    parser.add_argument(
        "--train-dataset",
        type=str,
        default="./flickr30k_splits/train",
        help="Path to training dataset",
    )
    parser.add_argument(
        "--eval-dataset",
        type=str,
        default="./flickr30k_splits/eval",
    ...
    # LoRA config
    parser.add_argument(
        "--lora-r", type=int, default=8, help="LoRA rank"
    )
    parser.add_argument(
        "--lora-alpha", type=int, default=8, help="LoRA alpha"
    )
    parser.add_argument(
        "--lora-dropout",
        type=float,
