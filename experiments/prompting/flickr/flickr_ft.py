...
def main(args):
    ...
    flickr30k = load_dataset("nlphuji/flickr30k", split="test")
    print("✅ Loaded:", len(flickr30k))

    # --- Filter dataset ---
    # keep only samples that are NOT in exclude_indices
    train_dataset = flickr30k.select(
        [i for i in range(len(flickr30k)) if i not in exclude_indices]
    )
