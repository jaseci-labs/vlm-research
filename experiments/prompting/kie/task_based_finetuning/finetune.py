...
def finetune_model(
    ...
    def convert_to_conversation(sample):
        """
        Convert KIE sample to conversation format.

        CHANGE FOR TASK:
        - Supervise ONLY the final total
        """
        ...
        # Use only total_amount
        total_value = ""
        if isinstance(annotations, dict):
            total_value = annotations.get("total_amount", "")

        target_text = json.dumps({"Total": str(total_value)})
